...
export function ActivityFeed({ className }: ActivityFeedProps) {
  ...
  const loadActivities = useCallback(
    async (filter: string = 'all', loadOffset: number = 0, append: boolean = false) => {
      try {
        if (!append) setLoading(true);

        const activityTypes =
          filter === 'all' ? ['watched', 'review', 'watchlist', 'list'] : [filter];

        const { activities: newActivities, hasMore: moreAvailable } = await getFilteredActivity(
          activityTypes,
          20,
          loadOffset
        );
