...
import { createClient, getCustomListWithItems, removeItemFromList, type CustomListWithItems, type ListItem } from "@/lib/supabase/client"
...
import { ArrowLeft, Edit, Trash2, Plus, Eye, EyeOff, Users, Calendar, Star, Film, Tv, GripVertical } from "lucide-react"
...
export function ListDetailContent({ listId }: ListDetailContentProps) {
  ...
  const getVisibilityIcon = (visibility: string) => {
    switch (visibility) {
      ...
      case 'followers': return <Eye className="h-4 w-4" />
      ...
      {list.banner_image_url && (
        <div className="aspect-[3/1] relative rounded-lg overflow-hidden">
          ...
          <div className="absolute bottom-4 left-4 text-white">
            ...
            {list.description && (
              <p className="text-white/90 max-w-2xl">{list.description}</p>
            )}
          </div>
        </div>
      )}
