...
export function handleClientError(error: unknown, context?: string): string {
  logger.error(`Client error in ${context || 'unknown'}`, error instanceof Error ? error : new Error(String(error)))
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt === maxRetries) {
        logger.error(`Operation failed after ${maxRetries} attempts in ${context || 'unknown'}`, lastError)
        throw lastError
      }
      
      logger.warn(`Operation failed, retrying (${attempt}/${maxRetries}) in ${context || 'unknown'}`, { attempt }, lastError)
